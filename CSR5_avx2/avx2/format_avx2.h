                               ...
                               const ANONYMOUSLIB_IT  *row_pointer)
{
    ...
    generate_partition_pointer_s1_kernel(row_pointer, partition_pointer, sigma, p, m, nnz);

    // step 2. check empty rows
    generate_partition_pointer_s2_kernel(row_pointer, partition_pointer, p);
                                             ...
                                             const iT      p)
{
    ...
    for (int par_id = 0; par_id < p-1; par_id++)
    {
        ...
        scan_single(s_segn_scan, ANONYMOUSLIB_CSR5_OMEGA + 1);
                                  ...
                                  ANONYMOUSLIB_IT        *_num_offsets)
{
    ...
    if (partition_descriptor_offset_pointer[p])
        scan_single(partition_descriptor_offset_pointer, p+1);
...
{
    generate_partition_descriptor_offset_kernel(row_pointer, partition_pointer,
...
void _aosoa_transpose_kernel_smem_IT(ANONYMOUSLIB_IT        *d_data,
                                     const uiT *d_partition_pointer,
                                     const int  nnz,
                                     const int  sigma,
                                     const bool R2C) {
    ...
    int num_thread = omp_get_max_threads();
    ANONYMOUSLIB_IT *s_data_all = (ANONYMOUSLIB_IT *) _mm_malloc(sigma * ANONYMOUSLIB_CSR5_OMEGA * sizeof(ANONYMOUSLIB_IT) * num_thread,
                                     ANONYMOUSLIB_X86_CACHELINE);
    #pragma omp parallel for
    for (int par_id = 0; par_id < num_p; par_id++) {
        int tid = omp_get_thread_num();
        ANONYMOUSLIB_IT *s_data = &s_data_all[sigma * ANONYMOUSLIB_CSR5_OMEGA * tid];
...
void _aosoa_transpose_kernel_smem_VT(ANONYMOUSLIB_VT        *d_data,
                                     const uiT *d_partition_pointer,
                                     const int  nnz,
                                     const int  sigma,
                                     const bool R2C) {
    ...
    int num_thread = omp_get_max_threads();
    ANONYMOUSLIB_VT *s_data_all = (ANONYMOUSLIB_VT *) _mm_malloc(sigma * ANONYMOUSLIB_CSR5_OMEGA * sizeof(ANONYMOUSLIB_VT) * num_thread,
                                                                 ANONYMOUSLIB_X86_CACHELINE);
#pragma omp parallel for
    for (int par_id = 0; par_id < num_p; par_id++) {
        int tid = omp_get_thread_num();
        ANONYMOUSLIB_VT *s_data = &s_data_all[sigma * ANONYMOUSLIB_CSR5_OMEGA * tid];
...
int aosoa_transpose(const int           sigma,
                    ...
                    bool                R2C) {
    _aosoa_transpose_kernel_smem_IT(column_index, partition_pointer, nnz, sigma, R2C);
    _aosoa_transpose_kernel_smem_VT(value, partition_pointer, nnz, sigma, R2C);
